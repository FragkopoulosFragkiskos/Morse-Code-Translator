...
public class MorseCodeController {
    ...
    public String translateToMorse(String textToTranslate) {
        ...
        for(Character latter : textToTranslate.toCharArray()) {
            translatedText.append(morseCodeMap.get(latter) + " ");
        }
        return translatedText.toString();
    }

    // morse message - contains the morse message after being translated
    ...
    // send audio data to played to data line
    private void playBeep(SourceDataLine line, int duration){
        // create audio data
        byte[] data = new byte[duration * 44100 / 1000];

        for(int i = 0; i < data.length; i++){
            // calculates the angle of the sine wave for the current sample based an the sample rate and frequency
            ...
            // also in the contex of audio processing ,  a signed bytes is often used to represent audio data because it
...
// We'll use a hashmap to translate user into morse code
// a hashmap is a data structure that stores key/value pairs
// in this case, we'll use the as the key and the corresponding morse code as the value
